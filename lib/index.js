...
function normalizeConfig(raw) {
	...
	if (!Object.hasOwn(PERSONALITY_PRESETS, config.personality.preset)) config.personality.preset = "warm";
	if (typeof config.personality.custom !== "string") config.personality.custom = "";
...
class MemoryStore {
	...
	async save() {
		if (this.corrupt) throw new ApiError(500, "memory-corrupt", "memory file is corrupt — writes are disabled until it is repaired");
	...
	add(text, source, maxEntries) {
		return this.enqueue(async () => {
			...
			if (trimmed === "") throw new ApiError(400, "bad-request", "memory text must not be empty");
			if (trimmed.length > MEMORY_MAX_TEXT_LENGTH) throw new ApiError(400, "memory-too-long", `memory text must be at most ${MEMORY_MAX_TEXT_LENGTH} characters`);
			const existing = this.entries.find((entry) => entry.text === trimmed);
			if (existing !== void 0) return { id: existing.id, created: false, duplicate: true };
			const cap = Number.isInteger(maxEntries) && maxEntries >= 1 ? maxEntries : MEMORY_MAX_ENTRIES;
			if (this.entries.length >= cap) throw new ApiError(400, "memory-limit", `memory is full (max ${cap} entries)`);
	...
	update(id, text) {
		return this.enqueue(async () => {
			...
			if (this.corrupt) throw new ApiError(500, "memory-corrupt", "memory file is corrupt — writes are disabled until it is repaired");
			const trimmed = typeof text === "string" ? text.trim() : "";
			if (trimmed === "") throw new ApiError(400, "memory-empty", "memory text must not be empty");
			if (trimmed.length > MEMORY_MAX_TEXT_LENGTH) throw new ApiError(400, "memory-too-long", `memory entry must be at most ${MEMORY_MAX_TEXT_LENGTH} characters`);
			const target = this.entries.find((entry) => entry.id === id);
			if (target === void 0) throw new ApiError(404, "memory-not-found", "memory entry not found");
			...
			if (dup !== void 0) return { id, updated: false, duplicate: true };
			target.text = trimmed;
...
function selectMemoryEntries(entries) {
	...
	for (const entry of sorted) {
		if (selected.length >= MEMORY_MAX_INJECTED) break;
		...
		if (selected.length > 0 && total + cost > MEMORY_MAX_CHARS) break;
		selected.push(entry);
		total += cost;
	}
	return selected;
...
function createSectionComposer() {
	...
	return function buildSectionText(getConfig, store) {
		...
		if (cache !== null && cache.config === config && cache.revision === store.revision) return cache.text;
		const parts = [];
		const custom = config.customInstructions.trim();
		if (custom !== "") parts.push(`【用户自定义指令】\n${custom}`);
		const tone = personalityText(config);
		if (tone !== "") parts.push(`【回复风格】\n${tone}`);
...
function registerMemoryTools(ctx, store, getConfig, autoGenerate) {
	...
	if (autoGenerate) {
		...
		execute: async (args) => {
			const config = getConfig();
			if (!config.memory.enabled || !config.memory.autoGenerate) return { id: "", saved: false, duplicate: false };
...
function buildApi(ctx, store, getConfig, setConfig, syncMemoryTools, loaded, enqueueConfigWrite, getLoadError) {
	...
	return {
		...
		"config.update": async (payload) => {
			...
			if (payload === null || typeof payload !== "object") throw new ApiError(400, "bad-request", "payload must be an object");
			...
			return enqueueConfigWrite(async () => {
				...
				if (next.customInstructions.length > INSTRUCTIONS_MAX_LENGTH) throw new ApiError(400, "instructions-too-long", `custom instructions must be at most ${INSTRUCTIONS_MAX_LENGTH} characters`);
				if (next.personality.custom.length > PERSONALITY_CUSTOM_MAX_LENGTH) throw new ApiError(400, "personality-too-long", `custom personality text must be at most ${PERSONALITY_CUSTOM_MAX_LENGTH} characters`);
		...
		"memory.add": async (payload) => {
			...
			if (typeof text !== "string") throw new ApiError(400, "memory-text-invalid", "memory text must be a string");
		...
		"memory.delete": async (payload) => {
			...
			if (ids.length === 0) throw new ApiError(400, "memory-ids-required", "at least one memory id is required");
			// L1: ids must be well-formed UUIDs.
			const uuidRe = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
			if (ids.some((id) => !uuidRe.test(id))) throw new ApiError(400, "memory-id-invalid", "memory id must be a valid UUID");
		...
		"memory.update": async (payload) => {
			...
			if (id === "") throw new ApiError(400, "memory-id-required", "memory entry id is required");
